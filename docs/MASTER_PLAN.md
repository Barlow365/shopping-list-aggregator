...
## Intake Mode (First-Run)

| HEADER: Back | Intake Mode
| PROMPT: What do you want to buy?
| FOLLOW-UP: one question at a time
| END: Start Trip | View Pool | Invite

--------------------------------------------------------------------------------
--------------------------------------------------------------------------------
| AID: Assisted Item Definition (3-Panel)
--------------------------------------------------------------------------------
+--------------------------------------------------------------------------------------+
...
| TRIGGER: Add Items
| FLOW: AID -> Add to Pool -> prompt add next
| NOTE: Store mention becomes a tag, not a lock

## Trip Start Moment

| HEADER: Start Trip
| SELECT STORE: Kroger | Target | Home Depot
| PREVIEW: eligible pool items
| ACTION: Generate Trip

| TRIP REFINEMENT (AID)
| IF ambiguous item -> open AID before adding to trip
| ACTIONS: [Add to Trip] [Back]

## Trip Mode

| HEADER: Trip Checklist
| TO BUY | PURCHASED
| ACTION: Add Receipt
...
| HEADER: Receipt
| LINE ITEMS -> update price memory
